...
| `status`                      | `string`             | Status of the payment. Possible statuses: `pending`, `processing`, `sent`, `failed`, `canceled`, `reversal_required`, `reversal_processing`, `reversed`. See *Payment Statuses* for more info.                                                       |
| `fund_status`                 | `string`             | Fund status of the payment. Possible statuses: `pending`, `requested`, `clearing`, `failed`, `sent`, `unknown`. See *Payment Fund Statuses*.  This property is available only when [individual payments are retrieved](/api/core/payments/retrieve). |
