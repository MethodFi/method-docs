...
To simulate the amount of information required from the user in the `dev` and `sandbox` environments,
provide a phone number from the table below to dictate the minimum requirements.

For the `dev` and `sandbox` environments, you may provide any value for name, dob, and address.

:::caution
Providing a phone number in the `dev` and `sandbox` environments that is not listed
