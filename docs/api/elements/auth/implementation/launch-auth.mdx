---
title: Launch Auth Element
sidebar_label: Launch Auth Element
...
<ObjectPropList
  ...
  about={(
    <div>
      <p>
        Using the <code>element_token</code> retrieved from your backend server in the previous step, you can launch
      ...
      <h4>Communication between Auth and your app</h4>
      <p>
        ...
        All redirect URLs will have the scheme ("prefix") <code>methodelements</code>. The event source will be
  ...
  snippetsElement={(
    <div>
      ...
      <hr className="dotted-divider" />
      <CodeBlock title="Auth Specific Element Events">
        {'methodelements://auth?{event_data}'}
      </CodeBlock>
      <hr className="dotted-divider" />
      <CodeBlock title={(
        <div>
          <h4>Initialize Auth Element</h4>
