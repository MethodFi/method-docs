...
Capabilities determine whether an account can be used as a
source account or destination account. Upon creation,
an account's `payments:receive` capability is enabled,
allowing the account to receive payments (as a destination).
...
Verification enables an account's `payments:send` capability,
