...
- A Plaid integration with the [Plaid Auth API](https://plaid.com/docs/auth/) enabled, and [Plaid Liabilities API](https://plaid.com/docs/liabilities) optionally enabled.
...
➡️  See https://plaid.com/docs/api/products/#authget for more info.
...
➡️  See the [GET /merchants endpoint](/docs/api/core/merchants/list) for more info.
...
**3.** Plaid Auth will only provided you with the `mask` or the last 4 digits of a person's liability account, you will need to collect the remaining initial numbers by launching the [Link Element](/docs/api/elements/link/intro). First create a link `element_token` from your app's backend. See [create an element token](/docs/api/elements/link/implementation/create-token) for more info.
...
**4.** With the retrieved `element_token`, [launch the Link Element](/docs/api/elements/link/implementation/launch-link) in your app through a WebView.
```
https://elements.production.methodfi.com?token={element_token}
```

**5.** Then finally [exchange](/docs/api/elements/link/implementation/exchange-public-accounts) the `public_account_token` for an account after your user successfully completes the Link Element flow.
